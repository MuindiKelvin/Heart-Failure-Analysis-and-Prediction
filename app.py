...
def get_binary_input(prompt):
    user_input = st.radio(prompt, ('Yes', 'No'))
...
def determine_lifestyle_changes(predict_type, new_person):
    ...
    if predict_type > 0:
        ...
        for i in lifestyle_changes:
            result_str += f"\nPlease {i},"
        result_str += "\nThis can reduce your heart failure risk. ❤️"
...
for i in range(4):
    with col1 if i == 0 else col2 if i == 1 else col3 if i == 2 else col4:
        for j in range(i, len(input_keys), 4):
            # Check if the input key is a binary option or the 'Sex' field
            if input_keys[j] in ['Diabetes', 'Family History', 'Smoking', 'Obesity', 'Alcohol Consumption', 'Previous Heart Problems', 'Medication Use', 'Diet']:
                user_input[input_keys[j]] = get_binary_input(input_keys[j])
            elif input_keys[j] == 'Sex':
                sex_input = st.radio("Sex", ('Male', 'Female'))
                user_input[input_keys[j]] = 1 if sex_input == 'Female' else 0
            else:
...
if 'Smoking' in user_input and 'Diet' in user_input and user_input['Smoking'] == 1 and user_input['Diet'] == 0:
    st.warning("Note: Smoking and an unhealthy diet are correlated with higher heart attack risk. Consider quitting smoking and adopting a healthier diet.")
